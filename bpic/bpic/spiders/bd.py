...
lua_script = """
        function main(splash, args)
            ...
            splash:runjs(string.format("document.querySelector('#kw').value = '{}';", args.kwname))
...
class BdSpider(scrapy.Spider):
    ...
    def parse(self, response):
        infos = response.css('img.main_img').xpath('@data-imgurl').extract()
    ...
    def save(self, pic, keyword):
        ...
        sql = """insert into pics(title, extra, uri, source_url, source_url_md5, create_time, update_time, status, tag)
              ...
              """.format(
            "", "[]", uri_md5.hexdigest(), pic, source_url_md5.hexdigest(), int(now), int(now), 0, tag.encode('utf-8'), int(now)
        )
        try:
            # 执行sql语句
            ...
            logging.error("db error, e: {}".format(e.message))
            self.db.rollback()
